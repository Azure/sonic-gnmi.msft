...
ifneq ($(ENABLE_DIALOUT_VALUE),0)
	...
	$(SUDO_GO) CGO_LDFLAGS="$(CGO_LDFLAGS)" CGO_CXXFLAGS="$(CGO_CXXFLAGS)" $(GO) test -race $(TEST_FLAGS) -coverprofile=coverage-ipinterfaces.txt -covermode=atomic -mod=vendor -v github.com/sonic-net/sonic-gnmi/show_client/helpers/ipinterfaces
