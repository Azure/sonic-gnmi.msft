...
func (sc *ServiceChecker) saveCriticalProcessCache() {
	/* saveCriticalProcessCache saves containerCriticalProcesses to a cache file.
		Note: Go uses JSON serialization instead of Python's pickle.*/
	if !sc.needSaveCache {
		return
	}

	sc.needSaveCache = false
	if len(sc.containerCriticalProcesses) == 0 {
	...
	if _, err := os.Stat(CriticalProcessCache); err == nil {
		os.Remove(CriticalProcessCache)
	}

	data, err := json.Marshal(sc.containerCriticalProcesses)
	if err != nil {
		log.Errorf("Failed to marshal critical process cache: %v", err)
		return
	}

	if err := os.WriteFile(CriticalProcessCache, data, 0644); err != nil {
		log.Errorf("Failed to save critical process cache: %v", err)
	}
}
