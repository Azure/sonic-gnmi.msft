...
func parseProcessLine(line string) (*TopProcess, error) {
...
func getTopMemoryUsage(options sdc.OptionMap) ([]byte, error) {
        ...
        topSortedByMemoryJSON, err := json.MarshalIndent(snapshot, "", "  ")
        if err != nil {
                log.Errorf("Unable to marshal top snapshot to JSON: %v", err)
                return nil, err
        }
        return topSortedByMemoryJSON, nil
}
